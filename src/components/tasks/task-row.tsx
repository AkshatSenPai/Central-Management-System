    ...
    <div className="grid grid-cols-[2fr_auto_auto_auto] items-center gap-4 border-b border-[var(--border)] px-4 py-3 last:border-b-0 hover:bg-[var(--surface-2)]">
      <Link href={`/tasks/${row.id}`} className="min-w-0">
        ...
        {/* Overdue is carried on the row, so it has to be visible: the
            ...
            milestone in --bad, and a task reading identically to an on-time
            one contradicts it. Colouring the existing subtitle rather than
            adding a badge keeps the vocabulary lock untouched — the due
            clause it recolours is the very thing that is late. */}
        <p
          className={`truncate text-xs ${
            row.overdue ? "text-[var(--bad)]" : "text-[var(--text-3)]"
          }`}
        >
          {row.subtitle}
        </p>
