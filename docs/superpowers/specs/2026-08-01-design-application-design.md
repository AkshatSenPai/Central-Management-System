...
| D6 | ~~**No modal or drawer primitive.**~~ **REVERSED 2026-08-02 by the owner.** The original reasoning — Phase 3a ruled "no overlay primitive" (its D6), Quick add was built as a popover to honour it, and nothing in *this* phase needed one — held right up until the mockup was opened rather than grepped. It contains two modals, New task and New project, so the decision and the design could not both stand. The reversal is narrow and its boundary lives in `src/components/ui/modal.tsx`: **a modal is for content you commit to** — a form with a Cancel and a Create. Menus, popovers and pickers stay popovers; `<QuickAdd>` and `<AccountMenu>` are not dialogs and must not become dialogs, or D6 will have been reversed into nothing. Built on native `<dialog>` + `showModal()`, so the top layer, inert background, focus trap and Escape are browser guarantees rather than hand-rolled approximations. |
...
Drawer primitives. (The modal primitive was out of scope for *this* phase and was built in the one after it — see D6.) A toast system unless §D7's condition is met. Configurable accents and dark tones (D8). (`--mono` and monospace rendering were likewise deferred, then shipped with the icon work.) Visual-regression screenshots — considered and declined in favour of mechanical gates. Restyling of Phase 4/5/7 surfaces beyond the shared `PlaceholderPage`. Any change to data, queries, server actions or schema: **this phase adds no behaviour**, and the one exception is `board.tsx` gaining `viewTransitionName` attributes, which changes no logic.
